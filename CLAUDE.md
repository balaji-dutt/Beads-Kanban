...
- **This fork is the maintained line.** `main` carries the work and tracks `origin/main`; `package.json` holds the current version. It is not on the VS Code Marketplace — releases ship as a VSIX from this repo's GitHub Releases. The `upstream` remote (davidcforbes) is kept only so a revival would be noticeable; never merge or pull it into `main`. The `origin/integration/bd-fixes` branch is kept for the same kind of reason: it marks where the fixes stopped being a patch series and became this fork. It is not stale work awaiting cleanup — leave it.
